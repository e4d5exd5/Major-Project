...
def createModel(IMAGE_WIDTH, IMAGE_HEIGHT, IMAGE_DEPTH, IMAGE_CHANNEL):
    ...
    output_layer_6_conv = layers.Conv2D(filters=64, kernel_size=(3,3), activation='relu')(output_layer_5_SA)

    output_layer_7_flatten = layers.Flatten()(output_layer_6_conv)

    output_layer_7_drop = layers.Dropout(rate=0.4)(output_layer_7_flatten,training=True)

    output_layer_7_dense = layers.Dense(256, activation='relu')(output_layer_7_drop)

    output_layer_8_drop = layers.Dropout(0.4)(output_layer_7_dense,training=True)

    output_layer_8_dense = layers.Dense(128, activation='relu')(output_layer_7_drop)

    model = Model(inputs=input_layer, outputs=output_layer_8_dense)
